...
class DatabaseHelper {
  ...
  static const _databaseVersion = 1; // Increment this if you change the schema in the future
  ...
  static const columnIsDeleted = 'is_deleted'; // INTEGER: 0 for false, 1 for true
  ...
  Future<int> insertHydrationEntry(HydrationEntry entry) async {
    ...
    int localId = await db.insert(tableHydrationEntries, map, conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<HydrationEntry?> getHydrationEntryByLocalId(int localId) async {
    ...
    final maps = await db.query(
      ...
      where: '$columnId = ? AND $columnIsDeleted = 0', // Only get non-deleted entries
      whereArgs: [localId],
      limit: 1,
    );
    if (maps.isNotEmpty) {
  ...
  Future<List<HydrationEntry>> getHydrationEntriesForUser(String userId, DateTime startDate, DateTime endDate) async {
    final db = await instance.database;
    // Normalize dates to ensure consistent querying at day boundaries
    final startOfDay = DateTime(startDate.year, startDate.month, startDate.day, 0, 0, 0, 0);
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      tableHydrationEntries,
      where: '$columnUserId = ? AND $columnTimestamp >= ? AND $columnTimestamp <= ? AND $columnIsDeleted = 0',
      whereArgs: [userId, startIso, endIso],
      orderBy: '$columnTimestamp DESC',
    );
    logger.d("Found ${maps.length} entries in local DB for user $userId in range.");
  ...
  Future<List<HydrationEntry>> getUnsyncedNewOrUpdatedEntries(String userId) async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      tableHydrationEntries,
      where: '$columnUserId = ? AND $columnIsSynced = 0 AND $columnIsDeleted = 0',
      whereArgs: [userId],
    );
    logger.d("Found ${maps.length} unsynced new/updated entries for user $userId.");
    return maps.map((map) => HydrationEntry.fromDbMap(map)).toList();
  }

  // Update an existing HydrationEntry in the local DB using its localDbId.
  // Returns the number of rows affected.
  Future<int> updateHydrationEntryByLocalId(int localId, HydrationEntry entry) async {
    final db = await instance.database;
    final map = entry.toDbMap();
  ...
  Future<int> markHydrationEntryAsSynced(int localId, String firestoreId) async {
    final db = await instance.database;
    logger.d("Marking local entry (localId: $localId) as synced with Firestore ID: $firestoreId");
    return await db.update(
      tableHydrationEntries,
  ...
  Future<int> markHydrationEntryAsDeletedByLocalId(int localId) async {
    final db = await instance.database;
    logger.d("Marking local entry (localId: $localId) as deleted and unsynced.");
    return await db.update(
      tableHydrationEntries,
      {columnIsDeleted: 1, columnIsSynced: 0}, // Mark as deleted and requires sync
      where: '$columnId = ?',
      whereArgs: [localId],
    );
  }
  ...
  Future<List<HydrationEntry>> getDeletedUnsyncedEntries(String userId) async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      tableHydrationEntries,
      where: '$columnUserId = ? AND $columnIsDeleted = 1 AND $columnIsSynced = 0',
      whereArgs: [userId],
    );
  ...
  Future<int> updateGuestEntriesToUser(String guestId, String firebaseUserId) async {
    final db = await instance.database;
    logger.i("Updating local entries from guestId: $guestId to firebaseUserId: $firebaseUserId. Marking as unsynced.");
    return await db.update(
      tableHydrationEntries,
      {columnUserId: firebaseUserId, columnIsSynced: 0, columnFirestoreId: null}, // Reset Firestore ID and mark as unsynced
      where: '$columnUserId = ? AND $columnIsDeleted = 0', // Only update non-deleted guest entries
      whereArgs: [guestId],
    );
  }
  ...
  Future<int?> getLocalIdFromFirestoreId(String firestoreId, String userId) async {
    ...
    final maps = await db.query(
      tableHydrationEntries,
      columns: [columnId],
      where: '$columnFirestoreId = ? AND $columnUserId = ? AND $columnIsDeleted = 0',
  ...
  Future<int> upsertHydrationEntry(HydrationEntry entry, String userId) async {
    ...
    if (entry.id != null) { // entry.id is Firestore ID
      localId = await getLocalIdFromFirestoreId(entry.id!, userId);
    }

    final entryWithUser = entry.copyWith(userId: userId, isSynced: true); // Mark as synced as it's coming from/aligned with Firebase
    final map = entryWithUser.toDbMap();
    map.remove(columnId); // Don't include localId in the map for insert/update values

    if (localId != null) {
      // Update existing local entry
      logger.d("Upserting: Updating local entry (localId: $localId) for Firestore ID ${entry.id}");
      await db.update(tableHydrationEntries, map, where: '$columnId = ?', whereArgs: [localId]);
      return localId;
    } else {
      // Insert new local entry
      logger.d("Upserting: Inserting new local entry for Firestore ID ${entry.id}");
      return await db.insert(tableHydrationEntries, map, conflictAlgorithm: ConflictAlgorithm.replace);
    }
  }
}
